...
:!email: {author-email}
:revnumber: v1.5.0
:revdate: 2019-08-17T13:18:08+0900
:revmark: Add support for attribute with empty value in pageAttributes
:doctype: article
...
return Boolean. So non-boolean values other than date are converted to `false`.
...
[NOTE]
====
This process is not executed when `definesEmptyAttributes` option is `false`.
